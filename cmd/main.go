...
func main() {
	...
	courseRepository := course.NewRepo(l, db) // Importamos el Logger (l)

	// Al haber hecho lo de la capa de servicio. Va a necesitar recibir un servicio, nosotros debemos especificarlo
	courseService := course.NewService(l, courseRepository) // Este userService se lo debemos pasar al endpoint. En este caso, le pasamos el repository // Importamos el Logger (l)
	...
	handler := handler.NewCourseHTTPServer(ctx, course.MakeEndpoints(courseService, course.Config{LimPageDef: pagLimDef}))
